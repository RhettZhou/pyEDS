...
from pyEDS import config_tools as ctools
from pyEDS import io_utils as ioutls
...
def _is_valid_calculation(path, para):
    metafile = os.path.join(path, DEFAULT_META_FILE)
    configfile = os.path.join(path, para, DEFAULT_CONFIG_FILE)
    inputfolder = os.path.join(path, DEFAULT_INPUT_FOLDER)
    outputfolder = os.path.join(path, para, DEFAULT_OUTPUT_FOLDER)
...
class MatchSeries:
    ...
    def __init__(self, data=None, path=None, para=None, **kwargs):
        if data is not None:
            # we provide data, we try to create new
            self.__setup_new_calculation(data, path, **kwargs)
        elif path is not None and para is not None:
            # data is none, we try to load
            self.__load_calculation(path,para)
        else:
            raise ValueError(
                "Either data and/or a path to a calculation must be provided"
            )

    def __load_calculation(self, path, para):
        if not _is_valid_calculation(path, para):
            raise ValueError(f"Matchseries data {path} / {para} does not seem valid")
        self.path = path
        self.__metadata = _load_metadata(path)
        self.metadata["output_folder_path"] = para + '/' + DEFAULT_OUTPUT_FOLDER
        self.configuration = _load_configuration(path + '/' + para)
        self.__load_data()
    ...
    def run(self):
        ...
        self.save_data(self.input_data_file)    ## Rhett delete the rest of code, move down

    def run_win(self):                          ## Rhett added this function 20220922
    ...
    def __run_match_series(self):
        ...
        while True:
            ...
            print(output, end="")
            if p.poll() is not None or "Press enter to continue." in str(output):
                break
        # if p.returncode != 0:
    ...
    def load(path,para):
        ...
        ms = MatchSeries(path=path,para=para)
        return ms
    ...
    def apply_deformations_to_spectra(self, specmap, sum_frames=True, **kwargs):
        ...
        **kwargs: passed to dask's compute function
        ...
        if isinstance(specmap, da.core.Array):
            ...
            axes["axis-0"]["size"] = defdt.shape[0]
            axlist = [axes["axis-1"], axes["axis-2"], axes["axis-3"]]  # Rhett changed here
            if specmap._lazy:
